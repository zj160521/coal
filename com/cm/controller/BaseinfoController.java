    ...
    public Object configBaseinfo(@RequestBody Sensor sensor, HttpServletRequest request) {
        ...
        try {
            if (sensor.getId() > 0) {
                ...
                addDevLink(sensor);
                String remark = JSONObject.toJSONString(sensor);
                String operation2 = "增加传感器" + sensor.getAlais();
                loginManage.addLog(request, remark, operation2, 15130);
    ...
    public void addDevLink(Sensor sensor) {
        ...
        if (devlinkSensors != null && devlinkSensors.size() > 0) {
            ...
            for (DevlinkSensor d : devlinkSensors) {
                ...
                if (d.getSensor_type() == 71) {
                	if (sensor.getHasfloor() == 0) {
                        link.setLogic_exps(sensor.getUid() + ">=" + sensor.getUpper_level4());
                        link.setRepower_exps(sensor.getUid() + "<" + sensor.getUpper_level4());
                    }
                    if (sensor.getHasfloor() == 1) {
                        link.setLogic_exps(sensor.getUid() + "<=" + sensor.getFloor_level4());
                        link.setRepower_exps(sensor.getUid() + ">" + sensor.getFloor_level4());
                    }
                    link.setAction(CUTOUT_ACTION);
                    link.setParam("" + d.getAction());
                }
                if (d.getSensor_type() == 65) {
                	if (sensor.getHasfloor() == 0) {
                        link.setLogic_exps(sensor.getUid() + ">=" + sensor.getLimit_alarm());
                        link.setRepower_exps(sensor.getUid() + "<" + sensor.getLimit_alarm());
                    }
                    if (sensor.getHasfloor() == 1) {
                        link.setLogic_exps(sensor.getUid() + "<=" + sensor.getFloor_alarm());
                        link.setRepower_exps(sensor.getUid() + ">" + sensor.getFloor_alarm());
                    }
