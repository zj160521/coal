...
public class SensorAlarmreportService {
	...
	@Autowired
	private EquipmentFailureStatementService service2;
			...
			if(classes!=null&&classes.size()>0){
				...
				for(EnvClasses c:classes){
					...
					if(startlong>endlong){
						if(starttimelong>startlong||starttimelong<=endlong){
	...
	public List<SensorAlarmreport> getfaildata() throws ParseException{
		SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		List<EnvClasses> classes = envclassesDao.getAll();
		List<SensorAlarmreport> list = new ArrayList<SensorAlarmreport>();
		List<CommunicationVo> list2 = service2.getallfail();
	...
	public void addFailReport(List<SensorAlarmreport> list){
		dao.addFailReport(list);
	}
