...
//		LogOut.log.debug(list.size()+"-----------------------------模拟量馈电异常数据");
	...
	@Scheduled(cron = "0 3 * * * ?")
	public void addFailreport(){
		try {
			List<SensorAlarmreport> list = alarmreportService.getfaildata();
		LogOut.log.debug(list.size()+"-----------------------------设备故障数据数据");
			if(list.size()>0&&list!=null){
					alarmreportService.addFailReport(list);
			}
		} catch (Exception e) {
			e.printStackTrace();
			StringBuffer sb = new StringBuffer();
