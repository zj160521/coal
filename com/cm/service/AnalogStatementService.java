...
public class AnalogStatementService {
	...
	@Autowired
	private EnvClassesService envService;
	...
	public List<SensorReportsVo> getAnasByDay(NameTime nameTime) throws ParseException{
		...
		for(SensorReportsVo s:statlist){
			...
			s.setMidvalue(saveOneBitTwoRound(s.getMidvalue()));
	...
	public List<EnvClasses> getclass(NameTime nameTime){
		List<EnvClasses> list = envService.getAll();
		List<EnvClasses> list2 = dao.getAlarmClass(nameTime);
		List<EnvClasses> list3 = dao.getSensorClass(nameTime);
		List<EnvClasses> list4 = dao.getStateClass(nameTime);
		Map<String, EnvClasses> map = new HashMap<String, EnvClasses>();
		if(list!=null&&list.size()>0){
			for(EnvClasses s:list){
				s.setStart(s.getStart()+":00");
				s.setEnd(s.getEnd()+":00");
				map.put(s.getName()+s.getStart()+s.getEnd(), s);
			}
		}
		if(list2!=null&&list2.size()>0){
			for(EnvClasses s:list2){
				EnvClasses a = map.get(s.getName()+s.getStart()+s.getEnd());
				if(a==null){
					s.setStatus(3);
					map.put(s.getName()+s.getStart()+s.getEnd(), s);
					list.add(s);
